...
class FileCommitHistoryDialog(tk.Toplevel):
    ...
    def _run_visual_diff(self, hexsha):
        ...
        try:
            ...
            SW_OPT_SILENT = 1
            SW_OPT_IGNORE_ACTIVATION = 64   # DismissExternalReferences / IgnoreActivationAndSuppression
            SW_OPT_AUTO_MISSING = 128

            def open_document(file_path):
                print(f"Opening in SolidWorks (external references blocked): {file_path}", flush=True)
                ...
                try:
                    ...
                    sw_app.SetUserPreferenceIntegerValue(REF_PREF_SW_LOAD_EXT_REFS, REF_PREF_VAL_NONE)
                except Exception as pe:
                    print(f"Warning: failed to set swLoadExternalReferences=None: {pe}", flush=True)
...
class GIT4SWApp(tk.Tk):
    ...
    def populate_file_table(self):
        ...
        for item in self.tree.selection():
            ...
            if values:
                selected_paths.append(values[0].replace("\\", "/"))
        ...
        for file_info in self.files_data:
            ...
            if selected_path_norm == "All Files" or dir_name == selected_path_norm:
    ...
    def lock_file(self):
        ...
        files_to_lock = [self.tree.item(item, 'values')[0].replace("\\", "/") for item in selected_items]
        ...
        def run():
            ...
            try:
                ...
                success_count = 0
                errors = []
                results = self.git_service.batch_lock_files(files_to_lock, max_workers=10)
                for file_rel_path, success, message in results:
                    if success:
    ...
    def unlock_file(self):
        ...
        files_to_unlock = [self.tree.item(item, 'values')[0].replace("\\", "/") for item in selected_items]
            ...
            try:
                ...
                for file_rel_path in files_to_unlock:
                    ...
                    if locked_by_others:
                        ...
                        continue
                    files_to_unlock_batch.append(file_rel_path)

                if files_to_unlock_batch:
                    results = self.git_service.batch_unlock_files(files_to_unlock_batch, max_workers=10)
                    for file_rel_path, success, message in results:
                        if success:
    ...
    def force_unlock_file(self):
        ...
        def run():
            ...
            try:
                ...
                success_count = 0
                errors = []
                results = self.git_service.batch_unlock_files(files_to_unlock, max_workers=10, force=True)
                for file_rel_path, success, message in results:
                    if success:
    ...
    def save_version(self):
        ...
        files_to_save = [self.tree.item(item, 'values')[0].replace("\\", "/") for item in selected_items]
    ...
    def get_selected_file_abs_paths(self):
        ...
        for item in selected_items:
            values = self.tree.item(item, 'values')
            if values:
                file_rel_path = values[0].replace("\\", "/")
...
""".replace("__TARGET_PATH__", repr(target_path_fs))
            ...
            try:
                ...
                for line in proc.stdout:
                    ...
                    if clean_line:
                        ...
                        if clean_line.startswith("Error:") or clean_line.startswith("❌"):
                            self.task_queue.put(('log', (f"⚠️ {clean_line}", "warning"), None))
                        elif clean_line.startswith("Warning:"):
                            self.task_queue.put(('log', (f"⚠️ {clean_line}", "warning"), None))
                        elif any(kw in clean_line for kw in ["Opening document", "Traversing", "Traversal complete",
                                                              "Saving to Excel", "BOM Tree and Partlist",
                                                              "Closing documents", "BOM cleanup",
                                                              "Closing newly", "Closed via",
                                                              "BOM cleanup verified", "BOM cleanup OK",
                                                              "pre-existing", "Starting BOM"]):
    ...
    def open_export_dialog(self):
        ...
        def get_filtered_files_list():
            ...
            for item in self.tree.get_children():
                ...
                if vals:
                    visible_files.append(vals[0].replace("\\", "/"))
            ...
            try:
                ...
                def on_finish(returncode, jp):
                    ...
                    if returncode == 0:
                        ...
                        try:
                            lbl_status.config(text=f"Export Failed (Exit Code: {returncode})", fg="#dc2626")
                            lbl_file.config(text=f"❌ SolidWorks export process failed with exit code {returncode}.", fg="#dc2626", height=2)
                            btn_cancel.config(text="Close")
                            btn_cancel.config(command=lambda: progress_pop.destroy())
                        except Exception:
                            pass
        ...
        def info_action():
            ...
            # OK button
            btn_close = ttk.Button(info_pop, text="OK", command=save_and_close)
    ...
    def discard_changes(self):
        ...
        files_to_discard = [self.tree.item(item, 'values')[0].replace("\\", "/") for item in selected_items]
            ...
            try:
                    ...
                    if not was_open:
                        ...
                        if not locked_by_others:
                            ...
                            if not is_already_locked_by_us and not getattr(self, 'suppress_auto_lock', False):
                                files_to_auto_lock.append(rel_path)

                # 4. Detect closed files -> collect for batch auto-unlock
                auto_unlock_map = {}
                ...
                if files_to_auto_lock:
                    def batch_auto_lock():
                        self.increment_tasks()
                        try:
                            results = self.git_service.batch_lock_files(files_to_auto_lock, max_workers=5)
                            for path, success, msg in results:
                                if success:
                                    self.files_locked_by_us.add(path)
                                    self.task_queue.put(('sw_status', f"Automatically locked {path}", None))
                                else:
                                    self.task_queue.put(('silent_error', f"Auto-lock failed for {path}: {msg}", None))
                        finally:
                            self.decrement_tasks()
                        import time
                        time.sleep(0.75)
                        if self.bg_tasks_count == 0:
                            self.task_queue.put(('callback', None, self.refresh_file_list))
                    threading.Thread(target=batch_auto_lock, daemon=True).start()

                # 6. Batch auto-unlock in a single thread
                if auto_unlock_map:
                    def batch_auto_unlock():
                        self.increment_tasks()
                        try:
                        ...
                        if self.bg_tasks_count == 0:
                            self.task_queue.put(('callback', None, self.refresh_file_list))
                    threading.Thread(target=batch_auto_unlock, daemon=True).start()

                # 7. Build status message/dictionary for Dashboard
