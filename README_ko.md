...
* **대화형 Git 그래프 시각화 (Browse Graph 버튼)**: Version History Log 화면의 **[Graph]** 버튼 좌측에 **[Browse Graph]** 버튼을 제공합니다. 이 버튼을 클릭하면 `graphviz` 및 `svg-pan-zoom` 라이브러리를 활용하여 전체 Git 커밋 히스토리를 대화형 SVG 기반 HTML 단일 파일(`.backup/git_graph_interactive.html`)로 자동 생성한 뒤 기본 웹브라우저로 즉시 띄워 줍니다. 마우스 드래그 및 마우스 휠을 이용하여 그래프의 이동, 축소, 확대가 자유롭게 되며 브랜치와 태그 이름도 노드에 직관적으로 결합되어 시각적 추적이 대폭 용이해졌습니다.
