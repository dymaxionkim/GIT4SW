...
* **대화형 Git 그래프 시각화 (Browse Graph 버튼)**: Version History Log 화면의 **[Graph]** 버튼 좌측에 **[Browse Graph]** 버튼을 제공합니다. 이 버튼을 클릭하면 `pyvis` 라이브러리를 활용하여 전체 Git 커밋 히스토리를 대화형 3D 커밋 네트워크 그래프(`git_graph.html`)로 자동 생성한 뒤 기본 웹브라우저로 즉시 띄워 줍니다. 마우스를 커밋 노드 위에 올리면 해시, 작성자, 작성일, 요약 메시지를 말풍선으로 상세 조회할 수 있으며 브랜치 HEAD와 태그도 각각 초록색과 노란색 노드로 구분되어 시인성이 대폭 향상되었습니다.
