...
* **백그라운드 SolidWorks 대량 일괄 변환 (EXPORT 버튼)**:
  ...
  - **정밀한 CAD 변환 품질 제어**:
    ...
    - **Watchdog 타임아웃 보호**: 개별 도면 파일 변환 시 SolidWorks 엔진이 무한 대기 또는 교착 상태(Deadlock)에 빠져 2분(120초) 이상 지연될 경우, 감시 타이머가 멈춘 서브프로세스를 강제 종료하고 SolidWorks 프로세스를 즉시 `taskkill` 처리한 뒤, 새 인스턴스를 재생성하여 다음 순번 파일부터 안전하게 자동으로 변환을 재개합니다.
