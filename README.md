...
* **Interactive Git Graph Visualization (Browse Graph button)**: A **[Browse Graph]** button is provided to the left of the **[Graph]** button on the Version History Log screen. Clicking it utilizes the `pyvis` library to automatically generate the entire Git commit history as an interactive 3D network graph (`git_graph.html`) and immediately opens it in the default web browser. Hovering the mouse over any commit node displays its SHA-1 hash, author, date, and summary message in a popup tooltip. Branch HEAD and tag nodes are color-coded in green and yellow, respectively, to significantly improve visual clarity.
