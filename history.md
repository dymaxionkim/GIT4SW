...
* **Graphviz 기반 대화형 Git 커밋 그래프 브라우징 (Browse Graph)**:
  - 버전 이력 화면("History log")에 **[Browse Graph]** 버튼을 신설하고, `graphviz` 패키지를 활용하여 Git 저장소의 전체 커밋 히스토리를 SVG 포맷으로 변환한 뒤, 확대·축소·이동(Pan & Zoom) 기능이 탑재된 HTML 단일 파일(`git_graph_interactive.html`)로 자동 생성해 줍니다.
  - 해당 파일은 로컬 `.backup/` 폴더 내에 안전하게 생성되어 기본 웹브라우저를 통해 실행됩니다.
  - 마우스를 올릴 필요 없이 브랜치 이름과 태그 이름이 각 커밋 박스 노드 내부에 선명히 결합되며, 일반 커밋은 청색, 브랜치 머리(HEAD)는 녹색(굵은 외곽선), 태그는 황색 배경으로 가시성을 극대화하여 시각적으로 추적·제어가 매우 용이합니다.
  - **속도 및 성능 최적화**: Windows 환경에서 Scoop 패키지 관리자를 통해 설치된 Graphviz의 실행기 지연(Shim Overhead)을 우회할 수 있도록 실제 이진 바이너리 경로(`C:\Users\dhkima\scoop\apps\graphviz\current\bin`)를 탐색하여 환경변수(`PATH`) 최우선에 동적 주입함으로써 그래프 파일 생성 속도를 약 12배 이상 단축했습니다. 또한, 초대형 저장소에서 레이아웃 계산 부하 및 웹 브라우저 SVG 렌더링 지연을 미연에 방지하기 위해 렌더링 범위를 최신 300개 커밋으로 제안(Capping)하였습니다.
...
| **Git 히스토리 시각화** | 단순 `git log` CLI 명령어 출력 (Graph 버튼) | GUI Graph 터미널 출력 + Graphviz 및 svg-pan-zoom 기반 대화형 웹 브라우저 그래프 (Browse Graph, .backup에 저장) 지원 | 텍스트 기반 시각화 한계를 극복하고 브랜치 합류/분기 관계 및 커밋 메시지를 브라우저 상에서 드래그, 확대/축소하며 직관적 판별 |
