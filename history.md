...
* **Pyvis 기반 대화형 Git 커밋 그래프 브라우징 (Browse Graph)**:
  - 버전 이력 화면("History log")에 **[Browse Graph]** 버튼을 신설하고, `pyvis` 패키지를 활용하여 Git 저장소의 전체 커밋 히스토리를 3D 스타일의 인터랙티브 커밋 네트워크 그래프로 시각화해 주는 기능을 구축했습니다.
  - 해당 그래프는 각 커밋별 해시, 작성자, 작성일, 요약 메시지를 마우스 오버(Hover) 시 말풍선으로 정밀하게 표시해 주며, 브랜치 머리(HEAD) 노드는 초록색 및 대형 크기, 태그 노드는 노란색 등으로 다양하게 장식하여 웹브라우저에서 직접 시각적으로 추적·제어할 수 있습니다.
...
| **Git 히스토리 시각화** | 단순 `git log` CLI 명령어 출력 (Graph 버튼) | GUI Graph 터미널 출력 + Pyvis 기반 대화형 웹 브라우저 3D 그래프 (Browse Graph) 지원 | 텍스트 기반 시각화 한계를 극복하고 브랜치 합류/분기 관계를 마우스 오버 팝업과 함께 대화형으로 직관적 판별 |
